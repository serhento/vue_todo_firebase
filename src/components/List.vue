<template>
    <ul class="todo__list">
        <li
                :class="[{active: index === SELECTED_TASK || SELECTED_TASK===0 && item.img}, 'active--'+item.color]"
        ...
        >
            ...
            <span class="todo__list--name">
                {{item.name}}
                <span v-if="item && !item.img">{{`(${Object.values(item.tasks).length})`}}</span>
            </span>
            <svg @click.stop="deleteList(index)" v-if="!item.img" width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
